...
- When the simulated virtual clock (`POST /clock` or `POST /api/clock`) transitions across a calendar date boundary (e.g., advancing to a new day), it locates all `'booked'` appointments scheduled on that new date and inserts a reminder record per appointment into the `Outbox`.
